...
// ── PUT /api/admin/quest-missions/[id] ────────────────────────────────────────
export async function PUT(request, { params }) {
  ...
  try {
    const { rows } = await query(
      `UPDATE quest_content SET
         title_fa = $1, title_en = $2, description_fa = $3, description_en = $4,
         icon_type = $5, icon_value = $6, icon_size = $7,
         xp_reward = $8, mission_type = $9, total = $10,
         target_company_id = $11, is_active = $12, sort_order = $13,
         target_hall_name = $14, hall_match_mode = $15, hall_scan_count = $16,
    ...
    console.error('[admin/quest-missions PUT]', err.message);
    return NextResponse.json({ error: 'server_error' }, { status: 500 });
  }
}

// ── DELETE /api/admin/quest-missions/[id] ─────────────────────────────────────
export async function DELETE(request, { params }) {
  ...
  try {
    const { rowCount } = await query('DELETE FROM quest_content WHERE id = $1', [id]);
    if (!rowCount) return NextResponse.json({ error: 'Not found' }, { status: 404 });
    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error('[admin/quest-missions DELETE]', err.message);
    return NextResponse.json({ error: 'server_error' }, { status: 500 });
  }
}
