  ...
  try {
    ...
    const rows = await query(
      `SELECT id, title_fa, title_en, description_fa, description_en, hall_fa, hall_en,
       ...
       FROM panels
       WHERE is_visible = true AND event_id = $1
         AND ($2 = '' OR title_fa ILIKE $3)
         AND ($4 = '' OR hall_fa = $4)
         AND ($5 = '' OR DATE(starts_at) = $5::date)
       ORDER BY starts_at ASC NULLS LAST`,
      [currentEventId, search, `%${search}%`, hall, date]
    );

    // Unique halls for filter UI
    const hallsResult = await query(
      `SELECT DISTINCT hall_fa FROM panels
       WHERE is_visible = true AND hall_fa IS NOT NULL AND event_id = $1
       ORDER BY hall_fa`,
      [currentEventId]
    );
    ...
    const datesResult = await query(
      `SELECT DISTINCT DATE(starts_at)::text AS date FROM panels
       WHERE is_visible = true AND starts_at IS NOT NULL AND event_id = $1
       ORDER BY date`,
      [currentEventId]
    );
