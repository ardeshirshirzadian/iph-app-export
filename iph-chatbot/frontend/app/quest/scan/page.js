...
export default function QRScanPage() {
  ...
  const handleResult = useCallback(
    (text) => {
      ...
      const boothId = text.replace("IPH-BOOTH-", "");
      setScannedBooth(boothId);
      setStatus("success");
      stopScanner();

      setTimeout(() => router.replace("/quest"), 1800);
    },
      ...
      {status === "success" && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-6 z-10 px-8">
          ...
          <div className="text-center">
            ...
            <p className="text-white font-bold text-lg leading-8">{t(lang, "scan_success_title")}</p>
            {scannedBooth && (
              <p className="text-white/40 text-sm mt-1">ID: {scannedBooth}</p>
            )}
          </div>
