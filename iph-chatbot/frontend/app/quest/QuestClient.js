...
const MOCK_USER = { name: "اردشیر شیرزادیان", xp: 850 };
...
const BOOTHS = [
  { id: 1,  name: "داروسازی البرز",          xp: 10 },
  { id: 2,  name: "شرکت دارو پخش",           xp: 10 },
  { id: 3,  name: "داروسازی تهران شیمی",     xp: 5  },
  { id: 4,  name: "داروسازی اکسیر",          xp: 5  },
  { id: 5,  name: "داروسازی روز دارو",       xp: 10 },
  { id: 6,  name: "شرکت پخش رازی",           xp: 5  },
  { id: 7,  name: "داروسازی زهراوی",         xp: 10 },
  { id: 8,  name: "داروسازی سینا",           xp: 5  },
  { id: 9,  name: "داروسازی کوثر",           xp: 10 },
  { id: 10, name: "داروسازی لقمان",          xp: 5  },
  { id: 11, name: "تجهیزات پزشکی پارس",     xp: 5  },
  { id: 12, name: "شرکت فن‌آوران طب",       xp: 10 },
  { id: 13, name: "داروسازی امین",           xp: 5  },
  { id: 14, name: "داروسازی باختر",          xp: 10 },
  { id: 15, name: "شرکت بهداشت دارو",        xp: 5  },
  { id: 16, name: "داروسازی جابرابن‌حیان",  xp: 10 },
  { id: 17, name: "داروسازی حکیم",           xp: 5  },
  { id: 18, name: "تجهیزات طب ایران",        xp: 10 },
  { id: 19, name: "داروسازی دانا",           xp: 5  },
  { id: 20, name: "شرکت سلامت فارما",        xp: 10 },
];

const SCANNED_BOOTH_IDS = new Set([1, 3, 7]);
const MY_RANK = 1;
...
function BoothsBottomSheet({ open, onClose, title, isRTL }) {
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      >
        ...
        <div className="overflow-y-auto max-h-[65vh] px-4 py-3 space-y-2 pb-8">
          {BOOTHS.map((booth) => {
            const scanned = SCANNED_BOOTH_IDS.has(booth.id);
            return (
              <div
                key={booth.id}
                className={`flex items-center gap-3 rounded-2xl px-4 py-3 border transition-colors ${
                ...
                style={scanned ? undefined : { background: "var(--surface-2)", opacity: 0.6 }}
              >
                <span className="text-xs w-5 text-center flex-shrink-0" style={{ color: "var(--text-dim)" }}>
                  {booth.id}
                </span>
                <span className="flex-1 text-sm leading-7 font-medium"
        ...
        {activeTab === "missions" && (
          <>
            <div className="grid grid-cols-3 gap-3 mb-6">
              {[
                { label: labels.statXpLabel,      value: "۴۵", unit: "XP",    icon: "⚡",  onClick: null,                           highlight: false },
                { label: labels.statScannedLabel, value: "۱",  unit: "غرفه",  icon: "📍",  onClick: () => setBoothsOpen(true),      highlight: true  },
                { label: labels.statRankLabel,    value: "۱",  unit: "ام",    icon: "🏅",  onClick: null,                           highlight: false },
              ].map((stat) => (
                <div
                  key={stat.label}
